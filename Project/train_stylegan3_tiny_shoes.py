...
cpp_extension.load = lambda *a, **k: (_ for _ in ()).throw(RuntimeError("Disabled"))

# Setup
stylegan3_path = '/users/eleves-a/2023/roman.lendormy/multimodal_genai/Project/stylegan3'
...
c.training_set_kwargs = dnnlib.EasyDict(
    ...
    path='/Data/roman.lendormy/s2p_dataset_processed/shoes_128.zip', # chemin vers le dataset d'entrainement au format zip
    use_labels=False,
    max_size=None,
    xflip=True,
    resolution=128,
...
c.run_dir = '/Data/roman.lendormy/stylegan3_training_shoes'
...
# ======= Checkpoint de reprise (optionnel):==========
resume_path = '/Data/roman.lendormy/stylegan3_training_shoes/network-snapshot-011200.pkl'

if os.path.exists(resume_path):
    print(f"🔄 Reprise depuis {resume_path}")
    c.resume_pkl = resume_path
    c.resume_kimg = 11200 #<------- A ajuster selon le checkpoint chargé !
else:
